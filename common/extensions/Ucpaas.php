...
class Ucpaas
{
     ...
     * @throws Exception
     */
    public function chooseNumber($appId, $callee, $dstVirtualNum,$cardno,$name,$cityId,$requestId,$type = 'json')
    {
        //$url = self::BaseUrl . self::SoftVersion . '/Accounts/' . $this->accountSid . '/Calls/voiceCode?sig=' . $this->getSigParameter();
        $body = '';
        $virtualType = "0";//Type=0{客户同步} Type=1{平台分配},不传则默认为1
        $cardtype = "0";//证件类型{默认填写0；表示证件类型为身份证}
        $record = "1";//是否录音，0表示不录音，1表示录音。
        $maxAge = "0";//0--代表无限，绑定虚拟号的时长；
        $statusUrl = 'https://api.namaiche.com/v1/uc/status-call-back';
        $hangupUrl = 'https://api.namaiche.com/v1/uc/hang-call-back';
        $recordUrl = 'https://api.namaiche.com/v1/uc/record-call-back';
        $url = self::BaseUrl.self::SoftVersion.'/Accounts/'. $this->accountSid .'/safetyCalls/chooseNumber?sig='.$this->getSigParameter();
        if ($type == 'json') {
            $body_json = array(
                'appId' => $appId,
                'callee' => $callee,
    ...
    //解绑虚拟号
    public function unbindNumber($appId, $bindId,$cityId,$type = 'json')
    {

        $body = '';
        $url = self::BaseUrl.self::SoftVersion.'/Accounts/'. $this->accountSid .'/safetyCalls/unbindNumber?sig='.$this->getSigParameter();
        if ($type == 'json') {
            $body_json = array(
                'appId' => $appId,
                'bindId' => $bindId,
...
//        return $url;
    ...
    //设置业务号码,顾问再拨打客户电话前，绑定；---10几秒后，自动解绑；
    public function setCallerNumber($appId,$bindId,$caller,$cityId,$requestId,$type = 'json')
    {

        $body = '';
        $url = self::BaseUrl.self::SoftVersion.'/Accounts/'. $this->accountSid .'/safetyCalls/setCallerNumber?sig='.$this->getSigParameter();
        if ($type == 'json') {
            $body_json = array(
                'appId' => $appId,
                'bindId' => $bindId,
    ...
    /**发送彩印*/
    public function  sendColorSMS($clientId,$pwd,$mobile,$templateId,$param,$type = 'json')
    {
        $body = '';
        $url = 'https://api.ucpaas.com/sms-partner/access/b00cc6/templatesms';
        if ($type == 'json') {
            $body_json = array(
                'clientid' => $clientId,
                'password' => md5($pwd),
